...
@allure.feature("Страница с категорией 'Одежда'")
@allure.severity(allure.severity_level.NORMAL)
@allure.description("Тесты на странице с одеждой")
@pytest.mark.clothes_page
class TestClothesPage:
    @allure.title("Проверка отображения хедера на странице /clothes")
    @pytest.mark.parametrize("presta_shop_url", [ClothesPage.URL], indirect=True)
    def test_clothes_page(self, driver):
        expected_header_text = "clothes"

        assert_element_text_equals(
            ClothesPage(driver).header_text, expected_header_text
        )
        assert ClothesPage(driver).header.is_displayed(), f"Не отображается header"

    @allure.title("Проверка разделов в хлебных крошках")
    @pytest.mark.parametrize("presta_shop_url", [ClothesPage.URL], indirect=True)
    def test_breadcrumbs(self, driver):
    ...
    @allure.title("Проверка отображения подкатегорий")
    @pytest.mark.parametrize("presta_shop_url", [ClothesPage.URL], indirect=True)
    def test_subcategories(self, driver):
    ...
    @pytest.mark.parametrize("presta_shop_url", [ClothesPage.URL], indirect=True)
    def test_subcategories_clothes(self, driver):
    ...
    @allure.title("Переход в мужскую категорию")
    @pytest.mark.parametrize("presta_shop_url", [ClothesPage.URL], indirect=True)
    def test_click_on_men_categories(self, driver):
        expected_header_text = "men"
        expected_men_url = "4-men"
        ClothesPage(driver).click_to_men_categories_button()

        assert_element_text_equals(
            MenCategories(driver).header.text, expected_header_text
        )
    ...
    @allure.title("Переход в женскую категорию")
    @pytest.mark.parametrize("presta_shop_url", [ClothesPage.URL], indirect=True)
    def test_click_on_women_categories(self, driver):
